...
RUNTIME_OVERRIDES_FILE = settings.root_dir / "config" / "runtime-overrides.json"
...
def _norm_park_key(value: str) -> str:
    return str(value or "").replace("Ⓜ️", "").strip().lower()


def _get_runtime_value(key: str, default: str) -> str:
    raw = runtime_overrides.get(key)
    text = str(raw or "").strip()
    return text if text else default


def _runtime_park_enum(metro: str, address: str) -> str:
    raw_map = runtime_overrides.get("park_enum_map")
    if isinstance(raw_map, dict):
        for candidate in (_norm_park_key(metro), _norm_park_key(address)):
            value = str(raw_map.get(candidate) or "").strip()
            if value:
                return value
    return PARK_ENUM_MAP.get(metro.replace("Ⓜ️ ", "").strip(), "")


def _runtime_park_string(metro: str, address: str) -> str:
    raw_map = runtime_overrides.get("park_string_map")
    if isinstance(raw_map, dict):
        for candidate in (_norm_park_key(metro), _norm_park_key(address)):
            value = str(raw_map.get(candidate) or "").strip()
            if value:
                return value
    return f"{metro}, {address}"
...
async def _handle_menu_action(message: Message, state: FSMContext | None = None) -> bool:
    ...
    if text == "📍 Адреса автопарков":
        labels = [f"{metro}, {address}" for metro, address in OFFICES]
        await message.answer("Выберите удобный автопарк:", reply_markup=offices_kb(labels))
        return True
    ...
    if text == "📄 Текущая заявка":
        ...
        if not bitrix_lead_id:
            ...
            return True
        ok, _, semantic, status_id = await get_lead_status(settings, bitrix_lead_id)
        if not ok:
            await message.answer(f"Заявка №{bitrix_lead_id} найдена, но CRM временно недоступна. Попробуйте чуть позже.")
            return True
        status_text = "закрыта" if semantic.upper() == "F" else "в работе"
...
async def _ask_question(message: Message, state: FSMContext, question_index: int) -> None:
    ...
    if question_index == 0:
        question_text = (
            f"{question.text}\n\n"
            "Выберите удобный автопарк кнопкой ниже."
        )
        await message.answer(progress + question_text, reply_markup=park_choice_kb(OFFICES))
...
async def answer_text_handler(message: Message, state: FSMContext) -> None:
    if await _handle_menu_action(message, state):
        return
    data = await state.get_data()
    idx = int(data.get("current_question_index", 0))
...
@dp.callback_query(QuizState.answering, F.data.startswith("park:"))
async def answer_park_handler(cb: CallbackQuery, state: FSMContext) -> None:
    await cb.answer()
    data = await state.get_data()
    idx = int(data.get("current_question_index", 0))
    if idx != 0:
    ...
    raw_idx = cb.data.split(":", 1)[1]
    if not raw_idx.isdigit():
        return
    office_idx = int(raw_idx)
    if office_idx < 0 or office_idx >= len(OFFICES):
        return

    metro, address = OFFICES[office_idx]
    answers: dict[str, Any] = data.get("answers", {})
    answers[question.key] = f"{metro}, {address}"
    await state.update_data(answers=answers)

    await cb.message.answer(f"Вы выбрали автопарк: {metro}, {address}")
    await _go_next(cb.message, state)
...
async def contact_phone_handler(message: Message, state: FSMContext) -> None:
    ...
    if park_info:
        ...
        park_text = _runtime_park_string(park_metro, park_address)
        metro_clean = park_metro.replace("Ⓜ️ ", "").strip()
        park_enum = _runtime_park_enum(park_metro, park_address)
        park_title_part = metro_clean

    lead_status_id = _get_runtime_value("lead_status_id", "14")
    source_description = _get_runtime_value("source_description", f"Бот ТГ @{bot_profile.username or 'Youpiter_quiz_bot'}")
    ...
    park_string_field = _get_runtime_value("park_string_field", "UF_CRM_1745483677126")
    park_enum_field = _get_runtime_value("park_enum_field", "UF_CRM_1741343224057")
    if park_text:
        lead_fields[park_string_field] = park_text
    if park_enum:
        lead_fields[park_enum_field] = park_enum
    if email:
        lead_fields["EMAIL"] = [{"VALUE": email, "VALUE_TYPE": "WORK"}]

    duplicate_mode = _get_runtime_value("duplicate_mode", "always_new")
    force_new = duplicate_mode != "update_duplicates"
