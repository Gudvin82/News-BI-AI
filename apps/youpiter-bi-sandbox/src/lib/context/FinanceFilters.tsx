...
interface FinanceCtx {
  ...
  setFilters: (f: FinanceFilterState) => void;
...
export const finDateHelpers = { todayMsk, dateOffset, firstDayOfMonth };

function defaultFilters(): FinanceFilterState {
  const today = todayMsk();
  return { mode: "day", date: today, dateFrom: finDateHelpers.firstDayOfMonth(), dateTo: today, park: "", type: "" };
}

const FinanceFiltersContext = createContext<FinanceCtx>({
  filters: defaultFilters(), setFilters: () => {}, metrics: null,
  loading: true, error: null, refresh: () => {}, noSheets: false,
});

export function useFinanceFilters() { return useContext(FinanceFiltersContext); }
...
export function FinanceFiltersProvider({ children }: { children: React.ReactNode }) {
  ...
  const load = useCallback(async (f: FinanceFilterState) => {
    ...
    try {
      ...
      docs = (rawDocs as Array<{ url?: string; name?: string; sections?: string[]; mapping?: unknown }>)
        .filter((d) => d.url && (!d.sections || d.sections.includes("finance")))
    ...
    try {
      const res  = await apiFetch(`/api/finance/sheets?from=${from}&to=${to}`, {
  ...
  return (
    <FinanceFiltersContext.Provider
      value={{ filters, setFilters, metrics, loading, error, refresh, noSheets }}
    >
      {children}
    </FinanceFiltersContext.Provider>
  );
