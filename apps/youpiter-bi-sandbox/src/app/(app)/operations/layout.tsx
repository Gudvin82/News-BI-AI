...
import { OpsFiltersProvider, useOpsFilters, opsDateHelpers } from "@/lib/context/OpsFilters";
import type { OpsFilterState, DateMode } from "@/lib/context/OpsFilters";

function OpsFilterBar() {
  const { filters, setFilters, loading, refresh } = useOpsFilters();
  const { todayMsk, dateOffset, firstDayOfMonth } = opsDateHelpers;
  const today = todayMsk();

  function patch(p: Partial<OpsFilterState>) { setFilters({ ...filters, ...p }); }
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {filters.mode === "day" ? (
          <div className="flex items-center gap-1 flex-shrink-0">
            <button onClick={() => patch({ date: dateOffset(filters.date, -1) })}
            ...
            <button onClick={() => !isToday && patch({ date: dateOffset(filters.date, 1) })}
              disabled={isToday}
...
function OpsInner({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const showFilters = !pathname?.includes("/operations/cars") && !pathname?.includes("/operations/drivers");
  return <div>{showFilters && <OpsFilterBar />}{children}</div>;
}

export default function OperationsLayout({ children }: { children: React.ReactNode }) {
  return <OpsFiltersProvider><OpsInner>{children}</OpsInner></OpsFiltersProvider>;
}
