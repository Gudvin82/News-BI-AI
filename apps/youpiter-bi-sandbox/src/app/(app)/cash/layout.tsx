...
import { CashFiltersProvider, useCashFilters, cashDateHelpers } from "@/lib/context/CashFilters";
import type { CashFilterState, DateMode } from "@/lib/context/CashFilters";

function CashFilterBar() {
  const { filters, setFilters, loading, refresh } = useCashFilters();
  const { todayMsk, dateOffset, firstDayOfMonth } = cashDateHelpers;
  const today = todayMsk();

  function patch(p: Partial<CashFilterState>) { setFilters({ ...filters, ...p }); }
